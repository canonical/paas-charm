...
"""Fixtures for Spring Boot charm integration tests."""
import os
import pathlib
...
async def charm_file_fixture(
    ...
    charm_file = next(
        (f for f in pytestconfig.getoption("--charm-file") if "/spring-boot-k8s" in f), None
    )
    if not charm_file:
        charm_file = await ops_test.build_charm(PROJECT_ROOT / "examples/springboot/charm")
    elif charm_file[0] != "/":
...
async def spring_boot_app_fixture(
    charm_file: str, model: Model, spring_boot_app_image: str, postgresql_k8s
):
