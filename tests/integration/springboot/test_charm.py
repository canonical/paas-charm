...
def test_springboot_is_up(spring_boot_app: App, juju: jubilant.Juju):
    ...
    for unit in status.apps[spring_boot_app.name].units.values():
        ...
        response = requests.get(f"http://{unit.address}:{WORKLOAD_PORT}/hello-world", timeout=5)
        assert response.status_code == 200
        assert "Hello, World!" in response.text


def test_migration(spring_boot_app: App, request: pytest.FixtureRequest, juju: jubilant.Juju):
    """
