...
def test_default_secret_key(
    ...
    status = juju.status()
    secret_keys = [
        requests.get(f"http://{unit.address}:{WORKLOAD_PORT}/config/SECRET_KEY", timeout=10).json()
        for unit in status.apps[flask_app_with_configs.name].units.values()
    ]
...
@pytest.mark.usefixtures("update_config")
def test_invalid_flask_config(flask_app_with_configs: App, juju: jubilant.Juju, invalid_configs: tuple[str, ...]):
    """
    arrange: build and deploy the flask charm, and change flask related configurations
    ...
    """
    status = juju.status()
    app_status = status.apps[flask_app_with_configs.name]
    ...
    for unit in app_status.units.values():
        ...
        for invalid_config in invalid_configs:
            assert invalid_config in unit.workload_message
...
def test_rotate_secret_key(
    ...
    """
    status = juju.status()
    unit_ips = [unit.address for unit in status.apps[flask_app_with_configs.name].units.values()]
    ...
    # Run action
    result = juju.cli("run", leader_unit, "rotate-secret-key", "--format=json")
    action_result = json.loads(result)
    # The result structure depends on jubilant, but we should check success
    
    juju.wait(lambda status: jubilant.all_active(status, flask_app_with_configs.name))
...
def test_port_without_ingress(
    ...
    """
    service_hostname = f"{flask_app_with_configs.name}.{juju.model}"
    result = juju.cli("exec", "--unit", f"{flask_app_with_configs.name}/0", f"/usr/bin/getent hosts {service_hostname}")
    service_ip = result.strip().split()[0]
...
def test_with_ingress(
    ...
    """
    try:
        juju.integrate(flask_app_with_configs.name, traefik_app_name)
    except jubilant.CLIError as err:
        if "already exists" not in err.stderr:
            raise err
    
    juju.wait(lambda status: jubilant.all_active(status, flask_app_with_configs.name, traefik_app_name))

    status = juju.status()
    traefik_ip = list(status.apps[traefik_app_name].units.values())[0].address
    response = requests.get(
        f"http://{traefik_ip}/config/BASE_URL",
        headers={"Host": f"{juju.model}-{flask_app_with_configs.name}.{external_hostname}"},
        timeout=5,
    )
    assert response.status_code == 200
    assert response.json() == f"http://{juju.model}-{flask_app_with_configs.name}.{external_hostname}/"
...
def test_app_peer_address(
    ...
    """
    juju.cli("add-unit", flask_app_with_configs.name)
    juju.wait(lambda status: jubilant.all_active(status, flask_app_with_configs.name), timeout=10 * 60)

    status = juju.status()
    actual_result = set()
    for unit in status.apps[flask_app_with_configs.name].units.values():
        response = requests.get(f"http://{unit.address}:{WORKLOAD_PORT}/env", timeout=30)
        assert response.status_code == 200
        env_vars = response.json()
    ...
    for unit in status.apps[flask_app_with_configs.name].units.values():
        ...
        expected_result.add(
            f"{unit.name.replace('/', '-')}.{flask_app_with_configs.name}-endpoints.{juju.model}.svc.cluster.local"
        )
    assert actual_result == expected_result

    juju.cli("scale-application", flask_app_with_configs.name, "1")
    juju.wait(lambda status: jubilant.all_active(status, flask_app_with_configs.name), timeout=10 * 60)
