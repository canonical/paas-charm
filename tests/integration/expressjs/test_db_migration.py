...
async def test_db_migration(
    expressjs_app: Application, model: juju.model.Model, get_unit_ips, postgresql_k8s
):
    """
    ...
    await model.add_relation(expressjs_app.name, postgresql_k8s.name)
    await model.wait_for_idle(status="active", timeout=20 * 60)

    for unit_ip in await get_unit_ips(expressjs_app.name):
        assert requests.head(f"http://{unit_ip}:8080/tables/users", timeout=5).status_code == 200
        user_creation_request = {"username": "foo", "password": "bar"}
        response = requests.post(
            f"http://{unit_ip}:8080/users", json=user_creation_request, timeout=5
        )
