...
def test_rabbitmq_ha_integration(
    ...
    try:
        ...
        # Simulate failure: Remove/Stop the first RabbitMQ unit
        # This forces the app to use the RABBITMQ_HOSTNAMES fallback logic

        with jubilant_temp_controller(juju, lxd_controller, lxd_model):
            juju.remove_unit(f"{rabbitmq_app.name}/0")
            juju.wait(
...
def test_rabbitmq_ha_endpoints(
    juju: jubilant.Juju,
    app_fixture: str,
    rabbitmq_app_fixture: str,
    port: int,
