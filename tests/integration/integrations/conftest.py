...
@pytest.fixture(scope="module", name="minio_credentials")
def minio_credentials_fixture(
    juju: jubilant.Juju,
):
    return {
...
def minio_app_fixture(juju: jubilant.Juju, minio_credentials):
...
def s3_integrator_app_fixture(
    ...
    task = juju.run(f"{s3_integrator}/0", "sync-s3-credentials", minio_credentials)
