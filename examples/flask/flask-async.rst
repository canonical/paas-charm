...
In this tutorial, we'll create a simple Async Flask application and learn how to
...
.. note::
    This tutorial requires version ``1.5.4`` or later for Rockcraft. Check
...
Test the Flask applications io page using ``curl``. It should respond back after
2 seconds.
.. literalinclude:: code/flask-async/task.yaml
    :language: bash
...
The Flask application should respond with ``I/O task completed in 2 seconds``.
...
To enable async workers open the ``rockcraft.yaml`` file and uncomment the
``parts: `` line and the following lines:
...
.. literalinclude:: code/flask-async/time_app.py
    :language: python
